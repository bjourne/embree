...
namespace embree
{
  ...
  unsigned int ConvertSubdivMesh(RTCDevice device, ISPCSubdivMesh* mesh, RTCBuildQuality quality, RTCScene scene_out, unsigned int geomID)
  {
    ...
    rtcSetSharedGeometryBuffer(geom,
                               ...
                               mesh->numFaces);
    rtcSetSharedGeometryBuffer(geom, RTC_BUFFER_TYPE_HOLE,
                               0, RTC_FORMAT_UINT,
                               mesh->holes,
                               0, sizeof(unsigned int),
                               mesh->numHoles);
...
unsigned int ConvertInstance(RTCDevice device,
                             ISPCScene* scene_in,
...
extern "C" RTCScene ConvertScene(RTCDevice g_device,
                                 ISPCScene* scene_in,
                                 RTCBuildQuality quality)
{
    ...
    if (g_instancing_mode != SceneGraph::INSTANCING_NONE)
    {
        for (unsigned int i=0; i<scene_in->numGeometries; i++)
        {
            ...
            {
                ConvertInstance(g_device,
                                scene_in,
                                (ISPCInstance*) geometry,
                                scene_out, i);
            }
            else
                assert(false);
    ...
    else
    {
        for (unsigned int i = 0; i < scene_in->numGeometries; i++)
        {
            ISPCGeometry* geometry = scene_in->geometries[i];
            if (geometry->type == SUBDIV_MESH)
                ConvertSubdivMesh(g_device,
                                  (ISPCSubdivMesh*) geometry,
                                  quality,
                                  scene_out, i);
            else if (geometry->type == TRIANGLE_MESH)
                ConvertTriangleMesh(g_device,
                                    (ISPCTriangleMesh*) geometry,
                                    quality,
                                    scene_out, i);
            else if (geometry->type == QUAD_MESH)
                ConvertQuadMesh(g_device,
